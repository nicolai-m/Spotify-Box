...
class Renderer:
    ...
    def _draw_empty_state(self, ctx: RenderContext):
        ...
        if not ctx.has_network:
            ...
            self.settings_button_rect = None
            line1 = self._render_text_rotated('Verbinde Spotify mit der Malti-Box um Musik zu hören', self.font_medium, COLORS['text_secondary'])
            line1_rect = line1.get_rect(center=(center_x - 30, center_y))
            self.screen.blit(line1, line1_rect)

            line2 = self._render_text_rotated('Drücke + um ein/e Album/Playlist zu speichern', self.font_medium, COLORS['text_secondary'])
